...
std::vector<std::tuple<size_t, size_t>> divide(size_t listsize, size_t suggested_groupsize)
{
	...
	// size_t nrOfGroups = std::ceil(static_cast<float>(listsize) / suggested_groupsize);

	std::vector<std::tuple<size_t, size_t>> result;

	for (size_t i = 0; i < listsize; )
	{
		auto j = i + suggested_groupsize;
		if (j > listsize)
			j = listsize;
	...
	float pvCutOff, float binomCutOff, float effectSize)
{
	...
	parallel_for(transcripts.size(), [&](size_t i) {
		...
		float strength = (1.0f * s_wt * a_g) / (a_wt * s_g);
		for (auto &r : result.replicate)
...
template<size_t NC>
std::vector<SLDataPoint> SLdataPoints(const std::vector<Transcript>& transcripts,
...
std::vector<std::tuple<float,float>> SLAnalyzeConsistency(
...
{
	std::array<std::vector<InsertionCount>, 1> aggregatedControlInsertions;
	aggregatedControlInsertions[0].reserve(controlInsertions[0].size());

	for (size_t i = 0; i < controlInsertions[0].size(); ++i)
	{
		aggregatedControlInsertions[0].emplace_back(InsertionCount{
	...
	parallel_for(4, [&](size_t i) {
		try
		{
			normalizedControlInsertions[i] = normalize(controlInsertions[i], aggregatedControlInsertions, 500);
		}
		catch (const std::exception &e)
		{
			eptr = std::current_exception();
	...
	std::array<std::vector<InsertionCount>, 3> normalizedInsertions;

	parallel_for(3, [&](size_t i) {
		try
		{
			normalizedInsertions[i] = normalize(insertions[i], aggregatedControlInsertions, 500);
		}
		catch (const std::exception &e)
		{
			eptr = std::current_exception();
	...
	std::vector<std::tuple<float,float>> result(transcripts.size());
	...
	// for (size_t i = 0; i < transcripts.size(); ++i)
	{
		...
		for (size_t j = 0; j < 3; ++j)
		{
			...
			{
				long v[2][2] = {
					{ normalizedInsertions[j][i].sense, normalizedInsertions[j][i].antiSense },
...
SLDataResult SLdataPoints(const std::vector<Transcript>& transcripts,
	const std::array<std::vector<InsertionCount>, 3>& insertions,
	const std::array<std::vector<InsertionCount>, 4>& controlInsertions,
	unsigned groupSize, float pvCutOff, float binomCutOff, float effectSize)
{
	auto ac = SLAnalyzeConsistency(transcripts, insertions, controlInsertions);
	// exit(0);
	...
	parallel_for(result.replicate.size(), [&](size_t i) {
		try
		{
			...
			// And now analyse this
			replicate.data = SLdataPoints(transcripts, ins, normalizedControlInsertions, groupSize);
